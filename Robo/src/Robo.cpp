    ...
    while(1)
    {
        if(CameraGetImageBuffer(hCamera,&sFrameInfo,&pbyBuffer,1000) == CAMERA_STATUS_SUCCESS)
		{
            ...
            if(!sz){
                ...
                const cv::Point centerC = std::move(cv::Point(frame.cols/2,frame.rows/2));
                cv::circle(frame,centerC,1,cv::Scalar(0,0,255),3);
                const auto [u,v] = getCenterOffset(std::move(centerP),std::move(centerC));
                cout<<centerP.x<<endl;
